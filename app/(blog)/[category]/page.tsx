...
export async function generateMetadata({ params }: ParamsProps): Promise<Metadata> {
  ...
  return {
    title: `${categoryData.name} - 리바이 개발블로그`,
    description: `${categoryData.name} 카테고리의 개발블로그 포스트들을 확인해보세요.`,
    keywords: [categoryData.name, '개발', '프론트엔드', '블로그', '개발블로그', '리바이'],
    openGraph: {
      type: 'website',
      title: `${categoryData.name} - 리바이 개발블로그`,
      description: `${categoryData.name} 카테고리의 개발블로그 포스트들을 확인해보세요.`,
...
export default async function CategoryPage({ params }: ParamsProps) {
  ...
  const jsonLd = {
    ...
    '@type': 'CollectionPage',
    name: `${categoryData.name} - 리바이 개발블로그`,
    description: `${categoryData.name} 카테고리의 개발블로그 포스트들을 확인해보세요.`,
