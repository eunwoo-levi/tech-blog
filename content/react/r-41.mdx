...
결과적으로, Lighthouse 접근성 및 SEO 점수가 각각 100점으로 개선되었고, VoiceOver로 검증한 결과도 훨씬 자연스러운 흐름이 되었다.
그 이유는, 시맨틱 태그와 ARIA로 UI 구조가 명확해지고, Tab 키 기반 키보드 네비게이션과 포커스 트래핑으로 모달에서 포커스 이탈 문제를 해결했기 때문이다.
