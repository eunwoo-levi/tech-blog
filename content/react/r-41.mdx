...
이번 글에서는 내가 실제 프로젝트에서 적용한 내용 기반으로 접근성 개선 과정을 단계별로 정리해보려고 한다.
...
## 2. aria-describedby + aria-invalid — 폼 상태 연결

구현하고 있는 서비스의 폼에서 가장 중요했던 건 에러 전달이었다.
...
위 코드에서 적용 포인트는 아래와 같다.

- `aria-invalid` → 현재 입력 상태 명시
- `aria-describedby` → 입력과 에러 연결
- `aria-live` → 동적 변화 낭독

결과적으로 에러는 더이상 “빨간 글씨”가 아니라 **상태 변화**로 인식되는 정보가 되었다.
...
모든 사용자가 더 편리하고 직관적인 웹 경험을 할 수 있도록 하는 기본 품질 향상 작업이다.

시맨틱 태그와 ARIA는 그 품질을 문서 구조 수준에서 끌어올리는 도구다.
그리고 VoiceOver와 같은 스크린 리더로 실제로 검증하는 과정이 필수적이다.

이번 글에서는 내가 실제 프로젝트에서 적용한 내용 기반으로 접근성 개선 과정을 단계별로 정리해보았다.
앞으로도 접근성을 지속적으로 개선하여 모두가 읽을 수 있는 서비스를 만들어 나가고자 한다.
