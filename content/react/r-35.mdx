...
# 그래서 선택한 방식: “React 외부 스토어 + useSyncExternalStore”
...
toastStore.setState()
      │
      ▼
listeners 실행 (구독자에게 알림)
      │
      ▼
useSyncExternalStore가 <Toast /> 리렌더 트리거
      │
      ▼
<Toast />가 최신 toasts 스냅샷 읽고 createPortal로 렌더
      │
      ▼
(duration 후)
removeToast() → setState() → 다시 리렌더 → 화면에서 제거
```
